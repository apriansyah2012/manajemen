...
$title = 'LAPORAN RESEP DI RAWAT INAP';
						  ...
						  <?php 
							...
							<tbody>
								...
								if (isset($_POST['jenis_bayar'])){
									...
									$query = "select a.tgl_peresepan, sum(b.status_poli ='Lama') as lama, sum(b.status_poli ='Baru') as baru, sum(d.kategori='TUNAI') as umum, sum(d.kategori IN ('PERUSAHAAN')) as pj, sum(d.kategori IN ('ASURANSI')) as asuransi,sum(d.kategori ='BPJS') as bpjs,sum(b.kd_pj ='A55') as karseh,sum(d.kategori ='JAMSOS') as kemkes, sum(c.jk ='L') as Laki,sum(c.jk ='P') as Perempuan, sum(b.stts='Dirujuk') as rujuk, sum(b.stts='Meninggal') as doa from resep_obat a Join reg_periksa b  join pasien c join penjab d where a.no_rawat=b.no_rawat and b.no_rkm_medis=c.no_rkm_medis and a.status ='Ranap' AND b.kd_pj=d.kd_pj  AND a.tgl_peresepan between '$tgl_awal' AND '$tgl_akhir'  
													GROUP BY
													a.tgl_peresepan"; 	
