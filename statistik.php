...
<?php
$date = $tahun."-".$bulan;
$_tgl_registrasi = "SELECT tgl_registrasi AS tanggal FROM reg_periksa WHERE tgl_registrasi LIKE '%$date%'  GROUP BY tgl_registrasi";
...
function getMorris(type, element) {
  if (type === 'line') {
      ...
      <?php while($data = fetch_array($hasil)){ ?>
      {
            ...
            baru: '<?php echo num_rows(query("SELECT status_poli AS baru FROM reg_periksa WHERE status_poli = 'Baru' AND tgl_registrasi = '{$data['tanggal']}' "));?>',
            lama: '<?php echo num_rows(query("SELECT status_poli AS lama FROM reg_periksa WHERE status_poli = 'Lama' AND tgl_registrasi = '{$data['tanggal']}' "));?>'
      },
      <?php } ?>
          ],
          xkey: 'tanggal',
      ...
      Morris.Donut({
          ...
          data: [{
              label: 'Sudah',
              value: <?php echo num_rows(query("SELECT no_rawat FROM reg_periksa WHERE tgl_registrasi LIKE '%$date%'  stts = 'Sudah'"));?>
          }, {
                  label: 'Batal',
                  value: <?php echo num_rows(query("SELECT no_rawat FROM reg_periksa WHERE tgl_registrasi LIKE '%$date%'  AND stts = 'Batal'"));?>
              }, {
                  label: 'Dirawat',
                  value: <?php echo num_rows(query("SELECT no_rawat FROM reg_periksa WHERE tgl_registrasi LIKE '%$date%' AND stts = 'Dirawat'"));?>
              }, {
                  label: 'Belum',
                  value: <?php echo num_rows(query("SELECT no_rawat FROM reg_periksa WHERE tgl_registrasi LIKE '%$date%' AND stts = 'Dirawat'"));?>
              }],
